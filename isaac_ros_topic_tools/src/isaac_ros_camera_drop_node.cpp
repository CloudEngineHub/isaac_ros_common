...
#include "isaac_ros_topic_tools/isaac_ros_topic_tools_common.hpp"
#include "isaac_ros_topic_tools/isaac_ros_camera_drop_node.hpp"
...
CameraDropNode::CameraDropNode(const rclcpp::NodeOptions & options)
: rclcpp::Node("drop", options)
{
  ...
  if (mode_ == modeToStringMap.at(CameraDropMode::Mono)) {
    ...
    exact_sync_mode_0_->registerCallback(
      std::bind(&CameraDropNode::sync_callback_mode_0, this, _1, _2));
    ...
    exact_sync_mode_1_->registerCallback(
      std::bind(&CameraDropNode::sync_callback_mode_1, this, _1, _2, _3, _4));
    ...
    exact_sync_mode_2_->registerCallback(
      std::bind(&CameraDropNode::sync_callback_mode_2, this, _1, _2, _3));
  } else {
    RCLCPP_ERROR(get_logger(), "Invalid mode: %s", mode_.c_str());
  }
}

std::vector<bool> CameraDropNode::create_evenly_spread_array(int array_size, int num_zeroes)
{
...
bool CameraDropNode::is_message_recent_enough(const rclcpp::Time & message_timestamp)
{
...
bool CameraDropNode::tick_dropper()
{
...
void CameraDropNode::sync_callback_mode_0(
...
void CameraDropNode::sync_callback_mode_1(
...
void CameraDropNode::sync_callback_mode_2(
  const sensor_msgs::msg::Image::ConstSharedPtr & image_ptr,
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & camera_info_ptr,
...
RCLCPP_COMPONENTS_REGISTER_NODE(nvidia::isaac_ros::topic_tools::CameraDropNode)
